...
auto animation::update(float to_value, float duration) -> void {
    ...
    this->value = resolve_value(ImGui::GetIO().DeltaTime, this->value, to_value, duration * multiplier);
}
...
auto animation_vec2::update(ImVec2 to_value, float duration) -> void {
    ...
    if (frame < 60)
        ...
        multiplier = frame / 60.f;
    this->value = {
            resolve_value(ImGui::GetIO().DeltaTime, this->value.x, to_value.x, duration * multiplier),
            resolve_value(ImGui::GetIO().DeltaTime, this->value.y, to_value.y, duration * multiplier)
    };
}
...
auto animation_vec4::update(ImVec4 to_value, float duration) -> void {
    ...
    this->value = {
            resolve_value(ImGui::GetIO().DeltaTime, this->value.x, to_value.x, duration * multiplier),
            resolve_value(ImGui::GetIO().DeltaTime, this->value.y, to_value.y, duration * multiplier),
            resolve_value(ImGui::GetIO().DeltaTime, this->value.z, to_value.z, duration * multiplier),
            resolve_value(ImGui::GetIO().DeltaTime, this->value.w, to_value.w, duration * multiplier)
    };
}
